...
/**
 * Converts human-readable unsigned decimals to exact scaled integers.
 */
final class ScaledDecimal
{
    /**
     * Parse a decimal without passing monetary values through floating point.
     */
    public static function parseUnsigned(string $value, int $scale): int
    {
        ...
        if (preg_match($pattern, $value) !== 1) {
            throw new InvalidArgumentException("The value must be an unsigned decimal with at most {$scale} decimal places.");
        }
    ...
    /**
     * Format a non-negative scaled integer for an editable decimal input.
     */
