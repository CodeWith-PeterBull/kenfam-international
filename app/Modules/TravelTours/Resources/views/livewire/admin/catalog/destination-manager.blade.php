            ...
            <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-4">
                            <section class="col-lg-5" aria-labelledby="travel-cover-title">
                                <h4 id="travel-cover-title" class="fs-16">Cover</h4>
                                @if($cover)
                                    <div class="travel-admin-media-item mb-3"><img src="{{ $cover->hasGeneratedConversion('card') ? $cover->getUrl('card') : $cover->getUrl() }}" alt="{{ $cover->getCustomProperty('alt_text', '') }}"><p>{{ $cover->getCustomProperty('alt_text', '') }}</p><div class="travel-admin-media-actions"><button type="button" class="btn btn-sm btn-outline-secondary" wire:click="openMediaMetadata({{ $cover->id }})">Edit text</button><button type="button" class="btn btn-sm btn-outline-danger" wire:click="removeCover" wire:confirm="Remove this destination cover?">Remove cover</button></div></div>
                                @endif
                                <form wire:submit="replaceCover" class="row g-2">
                                    <div class="col-12"><label for="travel-cover-upload" class="form-label">{{ $cover ? 'Replace cover' : 'Add cover' }}</label><input id="travel-cover-upload" type="file" accept="image/jpeg,image/png,image/webp" class="form-control @error('coverUpload') is-invalid @enderror" wire:model="coverUpload">@error('coverUpload')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    ...
                                    <div class="col-12"><label for="travel-cover-alt" class="form-label">Alternative text</label><input id="travel-cover-alt" class="form-control @error('coverAltText') is-invalid @enderror" wire:model.blur="coverAltText" maxlength="180">@error('coverAltText')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-12"><label for="travel-cover-caption" class="form-label">Caption</label><input id="travel-cover-caption" class="form-control @error('coverCaption') is-invalid @enderror" wire:model.blur="coverCaption" maxlength="320">@error('coverCaption')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                            ...
                            <section class="col-lg-7" aria-labelledby="travel-gallery-title">
                                <div class="d-flex justify-content-between gap-3"><div><h4 id="travel-gallery-title" class="fs-16 mb-1">Gallery</h4><p class="aureon-muted fs-12">{{ number_format($gallery->count()) }} / {{ config('travel-tours.media.destination_gallery_limit', 12) }} images</p></div></div>
                                <div class="travel-admin-media-grid mb-3">
                                    @forelse($gallery as $image)
                                        <article class="travel-admin-media-item" wire:key="travel-gallery-{{ $image->id }}"><img src="{{ $image->getUrl('thumb') }}" alt="{{ $image->getCustomProperty('alt_text', '') }}"><p>{{ $image->getCustomProperty('alt_text', '') }}</p><div class="travel-admin-media-actions"><button type="button" class="btn btn-icon btn-sm btn-outline-secondary" wire:click="openMediaMetadata({{ $image->id }})" aria-label="Edit image text"><i class="ti ti-pencil" aria-hidden="true"></i></button><button type="button" class="btn btn-icon btn-sm btn-outline-secondary" wire:click="moveGalleryImage({{ $image->id }}, 'up')" aria-label="Move image earlier"><i class="ti ti-arrow-left" aria-hidden="true"></i></button><button type="button" class="btn btn-icon btn-sm btn-outline-secondary" wire:click="moveGalleryImage({{ $image->id }}, 'down')" aria-label="Move image later"><i class="ti ti-arrow-right" aria-hidden="true"></i></button><button type="button" class="btn btn-icon btn-sm btn-outline-danger" wire:click="removeGalleryImage({{ $image->id }})" wire:confirm="Remove this gallery image?" aria-label="Remove image"><i class="ti ti-trash" aria-hidden="true"></i></button></div></article>
                                    @empty
                                        <p class="aureon-muted">No gallery images yet.</p>
                                    @endforelse
                                </div>
                                <form wire:submit="addGalleryImage" class="row g-2">
                                    <div class="col-12"><label for="travel-gallery-upload" class="form-label">Add gallery image</label><input id="travel-gallery-upload" type="file" accept="image/jpeg,image/png,image/webp" class="form-control @error('galleryUpload') is-invalid @enderror" wire:model="galleryUpload">@error('galleryUpload')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    @if($galleryUpload && method_exists($galleryUpload, 'temporaryUrl'))<div class="col-12"><figure class="travel-admin-upload-preview"><img src="{{ $galleryUpload->temporaryUrl() }}" alt="{{ $galleryAltText ?: 'Selected gallery preview' }}"><figcaption>{{ $galleryAltText }}</figcaption></figure></div>@endif
                                    <div class="col-md-6"><label for="travel-gallery-alt" class="form-label">Alternative text</label><input id="travel-gallery-alt" class="form-control @error('galleryAltText') is-invalid @enderror" wire:model.blur="galleryAltText" maxlength="180">@error('galleryAltText')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="travel-gallery-caption" class="form-label">Caption</label><input id="travel-gallery-caption" class="form-control @error('galleryCaption') is-invalid @enderror" wire:model.blur="galleryCaption" maxlength="320">@error('galleryCaption')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-12"><button type="submit" class="btn btn-primary" wire:loading.attr="disabled" wire:target="addGalleryImage,galleryUpload">Add image</button></div>
                                </form>
                            </section>
                        </div>
                    </div>
