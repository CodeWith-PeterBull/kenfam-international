                ...
                <tbody>
                    @forelse($destinations as $destination)
                        ...
                        <tr wire:key="travel-destination-{{ $destination->id }}">
                            <td><div class="d-flex align-items-center gap-3">@if($cover)<img class="travel-admin-thumb" src="{{ $cover->getUrl('thumb') }}" alt="{{ $cover->getCustomProperty('alt_text', '') }}">@else<span class="travel-admin-thumb travel-admin-thumb--empty"><i class="ti ti-photo" aria-hidden="true"></i></span>@endif<div class="min-w-0"><strong class="d-block text-break">{{ $destination->name }}</strong><small class="aureon-muted">{{ $destination->code ?: $destination->slug }}</small></div></div></td>
        ...
        <div class="modal fade show d-block" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="travel-media-metadata-title" wire:keydown.escape.window="closeMetadata">
            <div class="modal-dialog modal-md modal-dialog-centered" role="document">
                <form class="modal-content" wire:submit="saveMediaMetadata">
                    <div class="modal-header"><h3 id="travel-media-metadata-title" class="modal-title fs-18">Edit image text</h3><button type="button" class="btn-close" wire:click="closeMetadata" aria-label="Close image text form"></button></div>
