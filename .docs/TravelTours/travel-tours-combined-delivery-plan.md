...
### Storefront social identity — footer marks, tour sharing, and WhatsApp booking · 2026-09-20

**Delivered** on `feature/travel-tours-social-share`. The storefront now reads its social identity out of Institution Details the way the Commerce reference product page does: footer profile marks, a tour-page share group with pre-share context, and a "Book via WhatsApp" action.

| Area | Result |
|---|---|
| Institution contact | `Support/InstitutionContact` (module-local mirror of the Commerce helper, like `MoneyFormatter`): `whatsappNumber()` prefers a WhatsApp social entry and falls back to the primary phone, `whatsappBookingUrl()` prefills greeting, tour, from-price, and URL, `xHandle()` for `twitter:site`, `sameAs()` for structured data, and `socialLinks()` which recognises Facebook, Instagram, TikTok, X, YouTube, and LinkedIn from the recorded name (`fb`, `IG`, `Tik Tok`, `Twitter`…) or the URL host, skips WhatsApp (served by the call to action) and invalid URLs, and hands unknown platforms a generic link mark. `TravelStorefrontProfile` carries `socialLinks`, `xHandle`, `sameAs`; `whatsappNumber` now resolves through the helper. |
| Footer | `travel-footer__social`: one round mark per recorded profile under the descriptor, in recorded order, `rel="me noopener noreferrer"`, `aria-label="Kenfam on Facebook"`. Lucide 1.24 ships no brand glyphs, so `partials/brand-mark.blade.php` holds inline monochrome SVG marks (the Commerce approach) that inherit the theme colour; the row renders only when a link exists. |
| Pre-share context | Layout: `twitter:site` from the X handle and `sameAs` on the TravelAgency JSON-LD. Tour page: `product:price:amount/currency` from the default adult rate, `og:availability=instock` when a bookable departure exists, and the TouristTrip JSON-LD gains `offers` (URL, currency, price, availability, seller) and `provider.sameAs`. |
| Tour page actions | Hero actions row under the facts: **Check departures** (anchors `#travel-departures`; reads **Plan this journey** and anchors `#travel-inquiry` when nothing is bookable), **Book via WhatsApp** (`travel-button--whatsapp`, the institution number, prefilled text), and the share group (Facebook, X, WhatsApp intents in a popup; TikTok, Instagram, and Copy write the link to the clipboard with a toast; the native Web Share button is revealed where supported). Editorial previews are noindex and show neither. The inquiry aside keeps "Ask on WhatsApp". |
| Configuration | `travel-tours.storefront.sharing.{enabled,platforms}` (`TRAVEL_TOURS_SHARING_ENABLED`, `TRAVEL_TOURS_SHARING_PLATFORMS` allow-list) and `storefront.whatsapp_booking.enabled` (`TRAVEL_TOURS_WHATSAPP_BOOKING_ENABLED`), mirroring the Commerce keys. |

Files: `Support/InstitutionContact.php`, `Storefront/Data/SocialLink.php`, `Storefront/Data/TravelStorefrontProfile.php`, `Storefront/Services/TravelStorefrontProfileResolver.php`, `Config/travel-tours.php`, `Resources/views/layouts/storefront.blade.php`, `Resources/views/storefront/partials/{footer,share,brand-mark}.blade.php`, `Resources/views/storefront/catalog/show.blade.php`, `Resources/views/livewire/storefront/departure-selector.blade.php`, `Resources/assets/js/storefront.js`, `Resources/assets/css/storefront.css`, `.env.example`, `tests/Feature/TravelTours/TravelToursInstitutionContactTest.php` (4 tests), `tests/Feature/TravelTours/TravelToursStorefrontShareTest.php` (6 tests).
